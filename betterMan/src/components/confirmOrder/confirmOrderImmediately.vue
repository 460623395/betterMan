                                ...
                                <div class="orderListGoodsStyle">
                                    ...
                                    <div class="info">
                                        <div class="top">
                                            ...
                                            <p class="realPrice">￥{{(item.product_price).toFixed(2)}}
                                                <!--<br v-if="item.zkInfo.zk===true"/><span v-if="item.zkInfo.zk===true"-->
                                                <!--v-text="getOldPriceFn(item)"></span>-->
                                            </p>
                                        </div>
                                        <div class="bottom">
                                            <p class="gg" v-if="item.custom_option_info.length>0||Object.keys(item.custom_option_info).length>0">{{returnGoodSkuName(item)}}</p>
            ...
            goodsPirceCount() {
                ...
                for (let shop of this.confirmData.list) {
                    for (let good of shop.goods) {
                        count += (good.product_price *100* good.qty);
                    }
                }
                count = (count/100).toFixed(2);
                return `￥${count}`;
        ...
        methods: {
            ...
            countFinalPriceFn(){
                ...
                this.confirmData.list.forEach((shop,index,arr)=>{
                    ...
                    shop.goods.forEach((good)=>{
                        count+=good.product_price*good.qty*100;
                    });
                    //计算完商品原价以后看是否有选中优惠券
                    if(shop.userCanUseCoupons.length>0&&shop.couponSelected.id!==-1){
            ...
            shopPriceTotalFn(shop) {
                ...
                for (let key in shop.goods) {
                    total += shop.goods[key].product_price *100* shop.goods[key].qty;
                }
            ...
            confirmOrderFn() {
                ...
                if(this.orderNumCreatedSave===null){
                    ...
                    //     module_type:1,
                    //     //商品id
                    //     product_id:this.confirmData.list[0].goods[0].product_id,
                    //     //商品数量
                    //     qty:this.confirmData.list[0].goods[0].qty,
                    //     //商品sku，有，就返回，没有就是空
                    //     custom_option_sku:this.confirmData.list[0].goods[0].custom_option_sku,
                    //     //其余信息
                    //     post_info:{
                    //         //支付方式(默认支付宝)
                    //         payment_method:"alipay_standard",
                    //         //优惠券id
                    //         coupon_id:this.confirmData.list[0].couponSelected.id===-1?"":this.confirmData.list[0].userCanUseCoupons[this.confirmData.list[0].couponSelected.id].coupon_id,
                    //         //配送方式
                    //         shipping_method:this.confirmData.list[0].shippingMethod,
                    //         //店铺备注
                    //         order_remark:this.confirmData.list[0].remark
                    //     }
                    // };
                    // console.log(obj);
                    //请求接口
                    this.tools.globalFn.publicAxiosFn(this,{
                        needLogin:true,
                        url:"/checkout/onepage/submitimmediate",
                        data:{
                            //选择的地址id
                            address_id:this.shipAddress.address_id,
                            //类型：默认1
            ...
            getFirstData() {
                ...
                this.tools.globalFn.publicAxiosFn(this,{
                    needLogin:true,
                    url:"/checkout/onepage/immediate",
                    data:JSON.parse(JSON.parse(localStorage.shopSaveObject).buyImmediatelyNeedData),
                    success:(res)=>{
                        console.log("打印初始化订单");
                        console.log(res);
                        ...
                        if(res.data.code===200){
                            ...
                            this.confirmData.list.push({
                                //店铺id
                                shopId:res.data.data.cart_info.product.ushop_id,
                                //店铺名称
                                shopName:res.data.data.cart_info.product.shop_name,
                                //商铺下的商品列表
                                goods:[res.data.data.cart_info.product],
                                //该商铺下所有商品总计价格(尚未使用优惠券)
                                priceTotal:res.data.data.cart_info.product_total*100,
                                //店铺优惠券-可用的
                                userCanUseCoupons:res.data.data.cart_info.coupon_list===null?[]:res.data.data.cart_info.coupon_list.map((currentValue)=>{
                                    //减去价格乘以100
                                ...
                                userCantUseCoupons:[],
                                //店铺的配送方式
                                shippingMethod:res.data.data.cart_info.shipping_method,
                                //店铺备注
                                remark:"",
                                //店铺运费
                                shippingCost:0,
