<script>
    ...
    const userTokenInfo={
        //用户token
        token:"35f0b428a195c66a4e870290f5e1a9d215562393",
        //商城token
        shopToken:"qgCN2JjMQtMmB9YqzC1g2CwuekKBUFj3",
        //商城uuid
        shopUuid:"f3c2f0ae-5069-11e9-9aa6-00163e0561ee"
    };
    //公共信息
    ...
    const globalFn = {
        ...
        //统一的拿到用户token的方法
        publicGetUserNormalTokenFn(vue){
            //如果是开发模式，直接返回本地存储的token
            if(vue.tools.globalFn.regDevModeFn(vue)===1){
                return vue.tools.userTokenInfo.token;
            }
            //如果不是，返回存储在浏览器中的token
            return JSON.parse(localStorage.shopSaveObject).token;
        },
        //通用的请求方法
        publicAxiosFn(vue,options){
            ...
            * */
            ...
            // return;
            //如果当前是线上模式，那么除非needLogin是false，其他都默认加上商城token
            if(vue.tools.globalFn.regDevModeFn(vue)===0){
                vue.axios({
                    url:options.baseUrl+options.url,
                    method:options.method,
                    headers:options.needLogin===true?{
            ...
            else if(vue.tools.globalFn.regDevModeFn(vue)===1){
                //如果当前是线下模式，那么一切请求都加上默认配置的token，除非needLogin是false
                vue.axios({
                    url:options.baseUrl+options.url,
                    method:options.method,
                    headers:options.needLogin===false?undefined:{
                        "Fecshop-Uuid":vue.tools.userTokenInfo.shopUuid,
                        "Access-Token":vue.tools.userTokenInfo.shopToken
                    },
                    params:options.params,
                    data:options.data
                })
...
/*        publicAxiosFn(vue,options){
            ...
            vue.axios({
                ...
                headers:options.needLogin===false?undefined:{
                    ...
                    "Access-Token":"qgCN2JjMQtMmB9YqzC1g2CwuekKBUFj3"
                },
                params:options.params,
                data:options.data
            })
