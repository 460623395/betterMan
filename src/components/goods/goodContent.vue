<template>
    <div class="publicAllContentBg none" id="goodContent">
        ...
        <div class="goodJAndQBg">
            <van-loading type="spinner" color="black" v-if="contentAnmShowStatus.jPart===true"
                         class="globalLoadingAnmClass"></van-loading>
            <van-tabs v-model="goodJAndQData.tabActive" v-if="contentAnmShowStatus.jPart===false">
                <!--晒单评价-->
                <van-tab title="晒单评价">
                    <p class="noMoreDataLine paddingY" v-if="goodJAndQData.goodJList.length===0">暂无评价</p>
                    <ul class="evaListBox" id="evaListBox" v-if="goodJAndQData.goodJList.length>0">
                        <li v-for="(item,index) in goodJAndQData.goodJList" :key="index">
                            <div class="evaListStyle">
                                ...
                                <ul class="imgList" v-if="item.review_image.length>0">
                                    <li v-for="(img,index) in item.review_image" :key="index">
                                        <img :src="img" alt="">
                                    </li>
                                </ul>
                                <!--时间和规格-->
            ...
            <!--查看更多-->
            <div class="checkMore" v-if="contentAnmShowStatus.jPart===false&&goodJAndQData.goodJList.length>0" @click="JorQCheckMore">查看更多<van-icon name="arrow"></van-icon></div>
        </div>
        <!--店铺介绍-->
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            JorQCheckMore(){
                //判断目前选中的是哪个，跳转不同页面
                console.log(this.goodJAndQData.tabActive);
                switch (this.goodJAndQData.tabActive) {
                    case 0:
                        //跳转商品评价
                        this.$router.push({name:"evaList",params:{id:this.$route.params.id}});
                        break;
                    case 1:
                        //跳转商品问答
                        break;
                    ...
                    this.tools.globalFn.publicAxiosFn(this,{
                        ...
                        success:(res)=>{
                            ...
                            if(res.data.code===200){
                                ...
                                this.tools.globalFn.publicToastFn(this,"网络错误，请稍后再试");
                            }
                        }
                    })
                }
            ...
            publicSearchFecSkuFn(word){
                ...
                * */
                console.log(this.fecSkuSave.list);
            ...
            onAddCartClicked(skuData){
                console.log(skuData);
                console.log(this.goodSkuInfo.sku.none_sku);
                //先判断是否是无sku商品
                if(this.goodSkuInfo.sku.none_sku===true){
                    //如果是无sku商品，那么直接添加
                    this.tools.globalFn.publicAxiosFn(this,{
                        url:"/checkout/cart/add",
                        needLogin:true,
                ...
                // this.publicSearchFecSkuFn(skuData.selectedSkuComb.custom_option_key);
                // console.log(this.goodContentData);
            },
            //点击了立即购买的方法
                    ...
                    success:(res)=>{
                        if(res.data.code===200){
                            if(res.data.data.addressList.length>0){
                                if(this.goodSkuInfo.sku.none_sku===true){
                                    //如果是无sku商品，那么直接添加
                                    //把下一步需要初始化的信息，添加进本地存储
                                    this.tools.globalFn.publicChangeLocalStorageFn("buyImmediatelyNeedData",JSON.stringify({
                                        module_type:1,
                                        product_id:skuData.goodsId,
                                        qty:skuData.selectedNum,
                                        custom_option_sku:""
                                    ...
                                    //跳转立即购买确认订单页面
                                    this.$router.push({name:"confirmOrderImmediately"});
                                    return;
                                    // this.tools.globalFn.publicAxiosFn(this,{
                                    //     url:"/checkout/cart/add",
                                    //     needLogin:true,
                                ...
                                let result=this.publicSearchFecSkuFn(skuData.selectedSkuComb.custom_option_key);
                                if(result!==-1){
                                    console.log("打印最后出来的json");
                                    console.log({
                                        module_type:1,
                                        product_id:skuData.goodsId,
                                        qty:skuData.selectedNum,
                                        custom_option_sku:result.sku
                                    });
                                    //本地存储
                                    this.tools.globalFn.publicChangeLocalStorageFn("buyImmediatelyNeedData",JSON.stringify({
                                        module_type:1,
                ...
                this.tools.globalFn.publicAxiosFn(this,
                    {
                        ...
                        success:(res)=>{
                            ...
                            if(res.data.code===200){
                                ...
                                //放置评价数组
                                this.goodJAndQData.goodJList=res.data.data.product.reviewList;
                                //关闭评价动画
                                this.contentAnmShowStatus.jPart=false;
                                //顶部banner
                                ...
                                //放置sku组合
                                this.goodSkuInfo.sku.list=res.data.data.product.sku.list.map((el)=>{
                                    el.price*=100;
                                    return el;
                                });
