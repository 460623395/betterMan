<template>
    <div class="publicAllContentBg" id="myRewardList">
        <!--头部导航-->
        <van-nav-bar title="雇佣我的服务" class="globalNavBarHeaderClass"
                     left-text="返回"
                     left-arrow
                    ...
                    <li v-for="item in myServiceListData.list" :key="item.id">
                        <div class="orderListStyle">
                            ...
                            <div class="orderButtonBox">
                                ...
                                <span class="pay" @click="startSeviceConfirm(item)" v-if="item.order_status==1">开始服务</span>
    ...
    export default {
        ...
        methods: {
            ...
            goToOrderContentFn(item) {
                ...
                // alert(JSON.stringify(item));
                // this.$router.push("/userCenter/myRewardList/"+item.id);
                this.$router.push({name:"myHireMeServiceContent",params:{id:item.id}});
            },
            //判断当前页数是否等于总页数的方法
            isLastPageFn() {
